...
1. **Evaluation rigor** — until selection-on-test leakage, single-regime, and single-seed are fixed,
   no result is trustworthy. This is the prerequisite for _everything_: you can't tell signal from
...
The deep research (web, cited) should target #1 and #2 — _what does the evidence say works for
financial RL/ML and for trustworthy backtest evaluation_ — explicitly excluding what's already been
